...
case $dots in
  ...
  2) ips=$(for i in {0..255}; do echo $IP$i.; done)
      ...
      echo -e "\n\e[01;33mCasi 65536 IPs fueron generados\nLas solicitudes seran procesadas en shell.\e[0m\n\e[02m(PUEDE TARDAR...)\e[0m\n"
       x=$(($(wc -l < ips.txt)/3))
         ...
         $(for i in $(seq $x); do ip=$(sed -n "$i"p ips.txt); try=$(curl -m 3 -s -o /dev/null -w "%{http_code}" datacenter.vps-ssl.xyz -H "Upgrade: websocket" -x $ip:443); eval "case $try in $nums\ ) echo \"$ip|$try\" >> OK.txt; echo \"\e[01;33m$ip\e[0m | \e[01;37mIP OK - STATUS $try\e[0m\" >> ips2.txt; echo \"Creditos a @EhisOpeNer y a @alekc_2000 por la traduccion\" >> a;; *):; esac"; done; echo a >> b) </dev/null >/dev/null 2>&1&

         $(for i in $(seq $(($x+1)) $(($x*2))); do ip=$(sed -n "$i"p ips.txt); try=$(curl -m 3 -s -o /dev/null -w "%{http_code}" datacenter.vps-ssl.xyz -H "Upgrade: websocket" -x $ip:443); eval "case $try in $nums\ ) echo \"$ip|$try\" >> OK.txt; echo \"\e[01;33m$ip\e[0m | \e[01;37mIP OK - STATUS $try\e[0m\" >> ips2.txt; echo \"Creditos a @EhisOpeNer y a @alekc_2000 por la traduccion\" >> a;; *):; esac"; done; echo a >> b) </dev/null >/dev/null 2>&1&

         $(for i in $(seq $(($x*2+1)) $(wc -l < ips.txt)); do ip=$(sed -n "$i"p ips.txt); try=$(curl -m 3 -s -o /dev/null -w "%{http_code}" datacenter.vps-ssl.xyz -H "Upgrade: websocket" -x $ip:443); eval "case $try in $nums\ ) echo \"$ip|$try\" >> OK.txt; echo \"\e[01;33m$ip\e[0m | \e[01;37mIP OK - STATUS $try\e[0m\" >> ips2.txt; echo \"Creditos a @EhisOpeNer y a @alekc_2000 por la traduccion\" >> a;; *):; esac"; done; echo a >> b) </dev/null >/dev/null 2>&1&
  ...
  3) for i in {0..255}; do echo $IP$i >> ips.txt; done
      for i in $(seq $(wc -l < ips.txt)); do
         ...
         try=$(curl -m 3 -s -o /dev/null -w "%{http_code}" datacenter.vps-ssl.xyz -H "Upgrade: websocket" -x $ip:443)
          eval "case $try in
                 $nums\ )
